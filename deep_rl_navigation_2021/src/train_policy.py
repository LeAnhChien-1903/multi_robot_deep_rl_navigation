...
for j in range(hyper_params.ppo.E_phi):
    ...
    kl_divergence_batch = calculatePolicyKLDivergence(buffer.linear_probs_batch, 
                                                        buffer.angular_probs_batch, 
                                                        new_linear_probs, new_angular_probs).to(device)
    
    log_ratio = new_log_prob_batch - buffer.log_prob_batch
    ratio = log_ratio.exp()
    
    kl_divergence_new_old = kl_divergence_batch
    ...
    loss3 = -hyper_params.ppo.xi * torch.pow(torch.max(torch.zeros(1).to(device), kl_divergence_new_old - 2 * hyper_params.ppo.KL_target), 2)
