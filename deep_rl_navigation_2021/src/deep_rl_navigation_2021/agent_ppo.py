...
class Agent:
    ...
    def step(self, policy: ActorCritic, device):
        ...
        orient_obs = torch.from_numpy(orient_data.reshape(1, 2)).to(device)
        dist_obs = torch.from_numpy(dist_data.reshape(1, 1)).to(device)
    ...
    def setPoseInMapPlus(self):
        ...
        if id == 0:
            self.goal_pose = np.array([-1.0, 8.0, math.pi/2])
        elif id == 1:
            self.goal_pose = np.array([1.0, 8.0, math.pi/2])
        elif id == 2:
            self.goal_pose = np.array([-1.0, -8.0, -math.pi/2])
            ...
            self.goal_pose = np.array([-8.0, -1.0, math.pi])
        elif id == 5:
            self.goal_pose = np.array([-8.0, 1.0, math.pi])
        elif id == 6:
    ...
    def setColorInMapPlus(self):
        ...
        if id == 0:
            ...
            self.color = np.array([0.5, 1.0, 0.0])